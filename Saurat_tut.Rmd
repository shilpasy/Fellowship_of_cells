...
We'll then use this matrix to create a Seurat object. Think of this as creating a "fellowship of cells", like in Lord of the Rings, to go on an epic journey together.
...
```{r}
# FeatureScatter is typically used to visualize feature-feature relationships, but can be used 9# for anything calculated by the object, i.e. columns in object metadata, PC scores etc.
...
#Normalized values are stored here: #pbmc[["RNA"]]@data
```
...
Once we've identified our highly variable genes, we can use them to focus our downstream analysis and highlight the biological signal in our single-cell dataset. Keep up the great work, and don't forget that even the smallest heroes can have a big impact!
...
```
In this code, we're using the "ScaleData" function to shift the expression of each gene so that the mean expression across cells is 0, and scale the expression of each gene so that the variance across cells is 1. This step is important because it helps us compare gene expression levels between cells that might have different total amounts of RNA.
...
```
The "balanced" argument ensures that the color scale is symmetrical around zero.
...
2) The second approach is a statistical test based on a random null model, but it may not return a clear cutoff and is time-consuming for large datasets. 
This is very similar to Frodo's journey to Mount Doom, random walking and way too time-consuming with no clear end.
3) Finally, the third approach is a heuristic that is commonly used and can be calculated instantly, just like how Samwise used his intuition to make quick decisions.  
...
Canonical markers are often obtained through prior knowledge of cell type-specific genes or proteins, typically from previously published studies. These markers can also be identified through functional assays or immunohistochemistry. 
